...
export async function POST(req: NextRequest) {
    ...
    if (status === 'accept') {
        response = await supabase.from("invitations").update({
            is_accept: true,
            note: note
        }).eq("code", code)
            .is("is_accept", null)
    ...
    if (response?.error) {
        return createResponse({ type: "failed", status: response.status, error: response.error.message })
    }

    if (!response?.data) {
        return createResponse({ type: "failed", error: "data not found", status: 404 })
    }

    return createResponse({ type: "success", payload: response?.data, status: 200 })
}